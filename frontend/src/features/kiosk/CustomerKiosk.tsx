...
import { kioskCopy } from "../../config/languages";
...
export function CustomerKiosk() {
  ...
  if (!params.session || !params.token) {
    return (
      <div className="bg-surface font-sans text-on-surface min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <div className="text-h2 font-bold text-primary-container">Session link expired</div>
        <p className="mt-3 max-w-xl text-lg text-on-surface-variant">
          Please ask the staff member to reopen the customer screen from the active dashboard session.
        </p>
      </div>
    );
  }
  ...
  const copy = kioskCopy(kioskState.languageCode);
  const mainPrompt = kioskState.assistantText || copy.startPrompt;
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {typeof kioskState.sopResult.confidence === "number" && (
                  <div className={`rounded border px-2 py-1 text-[12px] font-bold ${kioskState.sopResult.confidence >= 0.6 ? 'border-green-300 text-green-700 bg-green-50' :
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex items-center gap-8">
            ...
            >
              ...
              <div className={`w-16 h-16 rounded-full flex items-center justify-center relative z-10 transition-all ${kioskState.isSessionActive ? 'bg-primary-container shadow-[0_0_20px_2px_rgba(0,45,98,0.2)]' : 'bg-slate-500 opacity-80'
                }`}>
          ...
          <div className="text-center">
            <span className={`font-sans font-bold uppercase tracking-widest text-lg text-[#002D62] block ${kioskState.isListening ? 'animate-pulse' : ''}`}>
              {kioskState.isSpeaking ? copy.aiSpeaking : kioskState.isListening ? copy.listening : kioskState.isSessionActive ? copy.activeSession : copy.tapToStart}
            </span>
            <span className="text-[13px] text-secondary uppercase tracking-[0.2em] mt-1 block">
              {kioskState.isSpeaking ? copy.pleaseListen : kioskState.isListening ? copy.pleaseSpeak : kioskState.isSessionActive ? copy.waitingForVoice : copy.startPrompt}
            </span>
          </div>
        </div>
